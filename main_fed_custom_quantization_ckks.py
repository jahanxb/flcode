...
def quantize_state_dict(state_dict, decimal_places=4):
    for key, value in state_dict.items():
        state_dict[key] = torch.round(value * (10**decimal_places)) / (10**decimal_places)
    return state_dict
...
if __name__ == '__main__':
    ...
    for t in range(args.round):
        ...
        for i in range(len(local_updates)):
            local_updates[i] = quantize_state_dict(local_updates[i], decimal_places=4)
        ##########################
        ...
        ### Custom Quantization
        aggregated_update = aggregate_quantized_updates(local_updates)
