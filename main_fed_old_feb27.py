...
if __name__ == '__main__':
    ################################### hyperparameter setup ########################################
    args = call_parser()
    local_model_size = list()
    global_model_size = list()
    torch.manual_seed(args.seed+args.repeat)
    torch.cuda.manual_seed(args.seed+args.repeat)
    np.random.seed(args.seed+args.repeat)
    ...
    for t in range(args.round):
        ...
        local_model_size.append(sys.getsizeof(local_updates))
        ##################### communication: avg for all groups #######################
        model_update = {
            k: local_updates[0][k] * 0.0
            for k in local_updates[0].keys()
        ...
        global_model_size.append(sys.getsizeof(global_model))
    ...
    print("local Model size: {:.3f}".format(sum(local_model_size)))
    
    print("global Model size: {:.3f}".format(sum(global_model_size)))
    
    import pickle
    #pickle.dump(global_model, open('/mydata/flcode/models/nodes_sftp/' + "1_global_model.pkl"))
    
    msg = pickle.dumps(global_model)
            
    torch.save(msg,f"/mydata/flcode/models/nodes_trained_model/global_models/1_gmodel.pkl")