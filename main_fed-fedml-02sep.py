...
class Sender(fedml_pb2_grpc.FedMlManagementServiceServicer):
    def __int__(self):
        self.gpu = -1
        self.num_user = 1
        self.batch_size = 50
    ...
    def argument_call(self, request, context):
        self.gpu = -1
        self.num_user = 1
        self.batch_size = 50
        self.round = 1
        ...
        return fedml_pb2.FedmlRequest(gpu=self.gpu, num_user=self.num_user, batch_size=self.batch_size,
                                      round=self.round, dataset=self.dataset
                                      )


def serve():
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=1))
    fedml_pb2_grpc.add_FedMlManagementServiceServicer_to_server(Sender(), server)
    server.add_insecure_port("[::]:9999")
    server.start()
    try:
        while True:
            print("Active Thread Count : %i" % (threading.active_count()))
            time.sleep(10)
